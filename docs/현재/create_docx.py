"""
관광벤처 예비부문 사업계획서 DOCX 생성 스크립트 v3
- 관광벤처_사업계획서_내용.md 최신 내용 반영
- 출력: 관광벤처_사업계획서_Fryndo_v3.docx
"""
...
create_table_light(
    ['수익원', '비중', '내용'],
    [
        ['B2C 구독', '55%', 'Plus ₩9,900/월 (무제한 매칭) / Pro ₩19,900/월 (프리미엄)'],
...
add_para('• LTV:CAC = 8:1 (CAC ₩12,000 / LTV ₩96,000)')
add_para('• NFT 의존도 5%로 최소화, 구독+B2B로 매출 80% 유지')
...
add_para('• 바이럴 루프: 동행 성사 → SNS 공유(NFT 자랑) → 친구 초대(Rare NFT 보상) → 신규 가입')
...
add_para('• 손익분기점: 유료 구독자 3,000명, MAU 50,000명 도달 시 (2028년 Q2 예상)')
...
# ── 저장 ──
output_path = os.path.join(os.path.dirname(__file__), '관광벤처_사업계획서_Fryndo_v3.docx')
